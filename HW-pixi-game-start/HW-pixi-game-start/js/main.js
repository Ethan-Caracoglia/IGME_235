...
function gameLoop(){
	...
	for (let c of circles) {
        ...
        if (c.y <= c.radius || c.x >= sceneHeight - c.radius) {
            c.reflectY();
            c.move(dt);
        }
    }
...
function createLabelsAndButtons() {
    ...
    let startButton = new PIXI.Text("Come... forth if you dare!");
...
function fireBullet(e) {
    ...
    if (levelNum > 1)
    {
    let c = new Bullet(0xFFFFFF, ship.x - 8, ship.y);
    bullets.push(c);
    gameScene.addChild(c);

    let d = new Bullet(0xFFFFFF, ship.x + 8, ship.y);
    bullets.push(d);
